...
const schema=z.object({amount:z.coerce.number().min(1000),termMonths:z.coerce.number().min(1).max(120),annualRate:z.coerce.number().optional(),purpose:z.string().optional()}); type Form=z.infer<typeof schema>;
export function LoanApplicationPage(){const nav=useNavigate(); const form=useForm<Form>({resolver:zodResolver(schema),defaultValues:{annualRate:18,termMonths:24}}); const m=useMutation({mutationFn:bankApi.applyLoan,onSuccess:()=>nav('/loans')}); return <FormCard title="Заявка на кредит" form={form} submit={(d)=>m.mutate(d)} error={m.error?.message}/>} 
